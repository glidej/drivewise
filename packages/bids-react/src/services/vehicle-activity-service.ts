...
export function nextVehicleActivityState(
  vehicle: Vehicle,
  state: VehicleActivityState,
  sequence: number,
): VehicleActivityState {
  return reduceVehicleActivityState(
    vehicle,
    state,
    buildSocketEvent(vehicle, vehicleSeed(vehicle), sequence),
  );
}

export function activityConnectionForElapsed(elapsedMs: number): VehicleActivityConnection {
  if (elapsedMs < 300) {
    return 'connecting';
  }

  if (elapsedMs >= 2100 && elapsedMs < 2600) {
    return 'reconnecting';
  }
...
export function withActivityConnection(
  state: VehicleActivityState,
  connection: VehicleActivityConnection,
): VehicleActivityState {
